...
client.on("messageCreate", async (message) => {
  ...
  // الحصول على البريفكس من الإعدادات أو استخدام البريفكس الافتراضي
  let settings = await client.music.get(message.guild.id);
  let prefix = settings?.prefix || botPrefix;
  
  // التأكد من أن البريفكس غير حساس لحالة الأحرف
  let mentionprefix = new RegExp(
    `^(<@!?${client.user.id}>|${escapeRegex(prefix.toLowerCase())})\\s*`, "i"
  );

  if (!mentionprefix.test(message.content.toLowerCase())) return;

  const [, nprefix] = message.content.toLowerCase().match(mentionprefix);
